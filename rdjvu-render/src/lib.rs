...
fn composite_mask_fg(w: u32, h: u32, mask: &Bitmap, fg: &Pixmap, page_w: u32, page_h: u32) -> Pixmap {
    ...
    for y in 0..h {
        for x in 0..w {
            if sample_mask(mask, x, y, w, h, page_w, page_h) {
                let (r, g, b) = sample_nearest(fg, x, y, w, h);
...
/// Nearest-neighbor sample from src layer, mapping output coords to src coords.
fn sample_nearest(src: &Pixmap, x: u32, y: u32, ow: u32, oh: u32) -> (u8, u8, u8) {
    let sx = ((x as f64 + 0.5) * src.width as f64 / ow as f64) as u32;
    let sy = ((y as f64 + 0.5) * src.height as f64 / oh as f64) as u32;
    src.get_rgb(sx.min(src.width - 1), sy.min(src.height - 1))
}
...
mod tests {
    ...
    fn debug_carte_layer_mismatch() {
        ...
        compare(
            &composite_bg_only(page.info.width as u32, page.info.height as u32, &bg),
